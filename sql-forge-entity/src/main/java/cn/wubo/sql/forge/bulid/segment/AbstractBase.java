...
import cn.wubo.sql.forge.bulid.enums.ManipulationType;
...
public abstract class AbstractBase<T, Children extends IBase<T, Children, R>, R> implements IBase<T, Children, R> {
    ...
    public AbstractBase(Class<T> entityClass, ManipulationType manipulationType) {
    ...
    protected void whereSql() {
        ...
        String whereSQL = conditions.stream().map(condition -> {
            ...
            // 检查条件是否为OR条件
            if (FunctionUtils.compileConditionOr(condition, t -> condition.conditionType() == EQ, t -> condition.conditionType() == NOT_EQ, t -> condition.conditionType() == GT, t -> condition.conditionType() == GTEQ, t -> condition.conditionType() == LT, t -> condition.conditionType() == LTEQ)) {
                params.put(params.size() + 1, condition.getValue());
                return condition.getColumn() + condition.conditionType().getValue() + "?";
            } else if (FunctionUtils.compileConditionOr(condition, t -> condition.conditionType() == LIKE, t -> condition.conditionType() == NOT_LIKE)) {
                String valueStr = "%" + condition.getValue() + "%";
                params.put(params.size() + 1, valueStr);
                return condition.getColumn() + condition.conditionType().getValue() + "?";
            } else if (condition.conditionType() == LEFT_LIKE) {
                String valueStr = "%" + condition.getValue();
                params.put(params.size() + 1, valueStr);
                return condition.getColumn() + condition.conditionType().getValue() + "?";
            } else if (condition.conditionType() == RIGHT_LIKE) {
                String valueStr = condition.getValue() + "%";
                params.put(params.size() + 1, valueStr);
                return condition.getColumn() + condition.conditionType().getValue() + "?";
            } else if (FunctionUtils.compileConditionOr(condition, t -> condition.conditionType() == BETWEEN, t -> condition.conditionType() == NOT_BETWEEN)) {
                ...
                return FunctionUtils.buildCondition(condition, t -> FunctionUtils.compileConditionAnd(t, tt -> tt.getValue() instanceof List, tt -> ((List<?>) tt.getValue()).size() == 2), t -> {
                    ...
                    return condition.getColumn() + condition.conditionType().getValue() + "? AND ?";
                });
            } else if (FunctionUtils.compileConditionOr(condition, t -> condition.conditionType() == IN, t -> condition.conditionType() == NOT_IN)) {
                // 处理IN和NOT IN条件
                return FunctionUtils.buildCondition(condition, t -> condition.getValue() instanceof List, t -> {
                    ...
                    if (valueObjs.isEmpty()) {
                        ...
                        return condition.getColumn() + condition.conditionType().getValue() + "(" + valueObjs.stream().map(obj -> "?").collect(Collectors.joining(",")) + ")";
                    }
                });
            } else if (FunctionUtils.compileConditionOr(condition, t -> condition.conditionType() == ConditionType.IS_NULL, t -> condition.conditionType() == IS_NOT_NUll)) {
                // 处理NULL和NOT NULL条件
                return condition.getColumn() + condition.conditionType().getValue();
            } else {
                return null;
