...
function ApiTemplateTabItem(props: { isCreate: boolean,apiTemplateId: string }) {
    ...
    useEffect(() => {
        if (!isCreate && apiTemplateId){
            apiClient.get(`/sql/forge/api/template?id=${apiTemplateId}`)
                .json()
                .then((data:unknown) => {
    ...
    const executeTest = () => {
        ...
        apiClient.post(`/sql/forge/api/template/${apiTemplateId}`, {json: params})
            .json()
            .then((data) => {
