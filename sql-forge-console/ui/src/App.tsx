...
import DatabaseTabItem from './databaseTabItem/DatabaseTabItem.tsx';
import {
  DeleteOutlined,
  EditOutlined,
  PlusOutlined,
...
function App() {
              ...
              if (tables) {
                tables.forEach(table => {
                  ...
                  if (indexes && indexes.length > 0) {
                    const indexNodes: DataType[] = indexes
                      .map(index => ({
                        title: index.indexName,
                        key: `ApiCalciteDatabaseSchemaTableTypeTableIndex-${schemaTableTypeTable.schema.tableSchema}-${tableType.tableType}-${table.table.tableName}-indexes-${index.indexName}`,
                        isLeaf: true
                      }))
                      .filter(
                        (item, index, self) =>
                      ...
                      .map(item => {
                        return {
                          ...
                          title: `${item.title}[${
                            indexes.find(
                              index => index.indexName === item.title
                            ).nonUnique
                              ? 'unique'
                              : 'nonUnique'
                          }(${indexes
                            .filter(index => index.indexName === item.title)
                            .map(index => index.columnName)
                            .join(',')})]`
                        };
                      });
                    const tableIndexNode: DataType = {
                      title: `索引`,
                      key: `ApiCalciteDatabaseSchemaTableTypeTableIndexes-${schemaTableTypeTable.schema.tableSchema}-${tableType.tableType}-${table.table.tableName}-indexes`,
                      children: indexNodes
                    };
                    tableNode.children?.push(tableIndexNode);
                  }
  ...
  const loadApiTemplate = async (TreeData: DataNode[]) => {
    ...
    try {
      const templates: {id: string}[] = await apiClient.get(
        '/sql/forge/api/template'
      );
    ...
    if (type === 'Database') {
      ...
      newPanes.push({
        ...
        children: (
          <ApiTemplateTabItem
            isCreate={false}
            apiTemplateId={type.substring(12)}
            reload={reloadApiTemplate}
          />
        ),
        key: newActiveKey
      ...
      newPanes.push({
        label: newLabel,
        children: (
          <ApiCalciteConfigTabItem
            remove={() => remove(newActiveKey)}
            reload={reloadApiCalcite}
          />
        ),
        key: newActiveKey
      });
      ...
      newPanes.push({
        ...
        children: (
          <ApiCalciteTabItem
            isCreate={false}
            apiTemplateId={type.substring(11)}
            reload={reloadApiTemplate}
          />
        ),
        key: newActiveKey
      ...
      newPanes.push({
        ...
        children: (
          <AmisTemplateTabItem
            isCreate={false}
            apiTemplateId={type.substring(13)}
            reload={reloadApiTemplate}
          />
        ),
        key: newActiveKey
  ...
  const removes = (targetKey: string) => {
    const newPanes =
      items?.filter(item => !item.key.startsWith(`Tab-${targetKey}-`)) || [];
    setItems(newPanes);
          ...
          <Tree
            ...
            titleRender={(nodeData: DataNode) => {
              ...
              // }
                ...
                nodeData.key.startsWith('DatabaseSchemaTableTypeTable-')
              ) {
                return (
                  <div>
                    <span>🧾{nodeData.title}</span>
                ...
                nodeData.key.startsWith('DatabaseSchemaTableTypeTableColumn-')
              ) {
                return (
                  <div>
                  ...
                  <div>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        apiClient
                          .delete(
                            `/sql/forge/api/template/${nodeData.key.substring(
                              12
                            )}`
                          )
                          .then(_ => {
                ...
                nodeData.key.startsWith('ApiCalciteDatabaseSchemaTableType-')
              ) {
                return (
                  <div>
                    <span>🔖{nodeData.title}</span>
                ...
                nodeData.key.startsWith(
                  'ApiCalciteDatabaseSchemaTableTypeTable-'
                )
              ) {
                return (
                  <div>
                ...
                nodeData.key.startsWith(
                  'ApiCalciteDatabaseSchemaTableTypeTableColumn-'
                )
              ) {
                return (
                  <div>
                  ...
                  <div>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        apiClient
                          .delete(
                            `/sql/forge/api/calcite/${nodeData.key.substring(
                              11
                            )}`
                          )
                          .then(_ => {
                  ...
                  <div>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        apiClient
                          .delete(
                            `/sql/forge/amis/template/${nodeData.key.substring(
                              13
                            )}`
                          )
                          .then(_ => {
                    ...
                    <Button
                      ...
                      onClick={() => {
                        // add(nodeData.key);
                        window.open(
                          `http://localhost:8080/sql/forge/amis?id=${nodeData.key.substring(
                            13
                          )}`
                        );
                      }}
