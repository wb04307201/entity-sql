...
public class ArgUtils {
     ...
     * @param message 错误信息
     * @param strs    字符串数组
     * @throws IllegalArgumentException 如果存在空字符串，则抛出异常
     */
    public static void isEmpty(String message, String... strs) {
        if (Arrays.stream(strs).anyMatch(str -> str == null || str.isEmpty())) {
            throw new IllegalArgumentException(String.format(IS_EMPTY_MESSAGE, message));
        }
    }
    ...
    public static void isEmpty(String message, String str) {
        if (str == null || str.isEmpty()) throw new IllegalArgumentException(String.format(IS_EMPTY_MESSAGE, message));
    }
